...
writing to standard output is a necessity in any programming language. from the earlier syntax, it might look very perculiar compared to other languages, as most other languages use functions to print. in c++, it is done with `cout` through stream operators `<<` which stream whatever is on the right side into standard output. if you would like to remember the direction, think of it as funneling input from your data **into** `cout`.
...
the code above will wait for the user to type in a number and press **enter**, in which case x will have the value of the input. similarly it looks like your data is coming from `cin` and going **into** your variable.
